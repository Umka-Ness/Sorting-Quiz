...
function feedback(opt, q) {
  const correctorno =
    data.answers[opt].find((a) => String(a).includes(q)) !== undefined;
  if (correctorno) {
    document.querySelector("#feedback").innerHTML =
      "<span>Congrats - you got that right!</span>";
  } else {
    document.querySelector("#feedback").innerHTML =
      "<span>Oh no - that was incorrect</span>";
  }
...
function nextslide() {
  num = Number(
    document
      .querySelector("#questions .slide:not(.hidden)")
      .id.split("slide")[1]
  );
...
function quizCompleted() {
  ...
  document.querySelector(
    "#end .detail"
  ).innerHTML = `You answered ${score.correct} out of ${score.total} questions correctly`;
}
function setupQuiz() {
  ...
  document.addEventListener("keydown", (e) => {
    if (
      !document.querySelector("#quiz").classList.contains("hidden") &&
      !document
        .querySelector("#questions")
        .classList.contains("animCircLeft") &&
      !document.querySelector("#questions").classList.contains("animCircRight")
    ) {
      switch (e.keyCode) {
        case 37:
          document
            .querySelector(".slide:not(.hidden) .optBtn:nth-of-type(1)")
            .click();
          break;
        case 39:
          document
            .querySelector(".slide:not(.hidden) .optBtn:nth-of-type(2)")
            .click();
          break;
        default:
          return;
...
function quizQuestionSetup() {
  ...
  for (const q in sortedQs) {
    questionHtml += `<div id="slide${q}" tabindex="0" class="slide ${
      Number(q) === 0 ? "" : "hidden"
    }">
    <fieldset>
      <legend class="lgnd"><span>${sortedQs[q][0]}</span></legend>
      <button class="optBtn" onclick="check('${allAs[0]}','${
      sortedQs[q][0]
    }');document.querySelector('#questions').classList.add('animCircLeft');" data-opt="${
      allAs[0]
    }" data-q="${sortedQs[q][0]}">${allAs[0]}</button>
      <button class="optBtn" onclick="check('${allAs[1]}','${
      sortedQs[q][0]
    }');document.querySelector('#questions').classList.add('animCircRight');" data-opt="${
      allAs[1]
    }" data-q="${sortedQs[q][0]}">${allAs[1]}</button>
    </fieldset>
  </div>`;
  }
  return questionHtml;
